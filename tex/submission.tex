...
\begin{answer}
  ...
  % TODO: Answer -- What does alpha=0.0 signify in terms of the CQL algorithm?
  % (With alpha=0.0 the CQL regularizer is disabled and the algorithm reduces
  % to standard DQN with no conservative penalty on OOD actions.)

  % TODO: Explain the performance difference between alpha=0.0 and alpha=0.1.
  % Hint: alpha=0.1 penalizes overestimation of OOD Q-values, leading to more
  % conservative and reliable value estimates in the offline setting.
...
\begin{answer}
  ...
  % TODO: Write 2-3 sentences: explain the performance gap between RND and
  % random exploration for offline CQL on PointmassHard-v0.
  % Hint: same argument as 1c -- RND provides better state coverage in the
  % offline dataset, which directly improves the quality of the learned policy.
